...
class LbaasPool(BaseAPI):
    ...
    Args:
        ...
        lbaas (object): Объект класса :class:`rcp.Lbaas`. Балансировщик, к
                          которому относится данный Lbaas Pool
        connlimit (str): лимит соединений  для пула балансировщика
        cookie_name (str): Имя cookie
        members (list): Список объектов класса :class:`rcp.LbaasPoolMember`.
    ...
    def get_object(cls, lbaas, pool_id):
        ...
        Args:
            lbaas :class:`rcp.Lbaas`
            pool_id (str): Идентификатор Lbaas Pool

        Returns:
            object: Возвращает объект LbaasPool :class:`rcp.LbaasPool`
        """
