...
class Network(BaseAPI):
    ...
    Args:
        ...
        subnets (object): Список объектов класса :class:`rcp.Subnet`
    ...
    class Meta:
        id = Field()
        name = Field()
        vdc = Field('rcp.Vdc', allow_none=True)
        is_default = Field()
        subnets = FieldList('rcp.Subnet')
    ...
    def get_object(cls, id):
        ...
        Returns:
            object: Возвращает объект сети :class:`rcp.Network`
        """
        network = cls(id=id)
    ...
    def add_subnet(self, subnet):
        ...
        Args:
            subnet (object): Объект подсети :class:`rcp.Subnet`
        """
        if subnet.id:
        ...
        subnet = self._call('POST', 'v1/network/{}/subnet'.format(self.id),
                            ...
                            dns_servers=[])
        self.subnets.append(resolve('rcp.Subnet')(token=self.token, **subnet))
