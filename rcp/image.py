from rcp.base import BaseAPI, Field, FieldList, ObjectAlreadyHasId, \
    ObjectHasNoId
from rcp.consts import DEFAULT_TIMEOUT, IMAGE_CREATE_TIMEOUT, \
...
class Image(BaseAPI):
    ...
    Args:
        id (str): Идентификатор
        name (str): Имя
        files (list): Список объектов класса :class:`rcp.File`. Список файлов в
                        образе
    ...
    def get_object(cls, id):
        ...
        Returns:
            object: Возвращает объект образа :class:`rcp.Image`
        """
        image = cls(id=id)
    ...
    def deploy_vm_from_image(self, vm):
        ...
        Args:
            vm (object) - объект создаваемого сервера :class:`rcp.Vm`

        Returns:
            object: объект созданного из образа сервера  :class:`rcp.Vm`
        """
