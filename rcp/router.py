...
class Router(BaseAPI):
    ...
    Args:
        ...
        name (str): Имя
        vdc (object): ВЦОД, к которому относится маршрутизатор :class:`rcp.Vdc`
        is_default (bool): True для маршрутизатора по умолчанию
        floating (object): Порт подключения к внешней сети :class:`rcp.Port`
    ...
    def get_object(cls, id):
        ...
        Returns:
            object: Возвращает объект маршрутизатора :class:`rcp.Router`
        """
        router = cls(id=id)
    ...
    def add_port(self, port):
        ...
        self.ports.append(resolve('rcp.Port')(token=self.token, **port))
