...
class Port(BaseAPI):
    ...
    Args:
        ...
        fw_templates (list): Включенные шаблоны брандмауэра
                             :class:`rcp.FirewallTemplate`
        network (object): Сеть :class:`rcp.Network`
    ...
    class Meta:
        ...
        type = Field()
        vdc = Field("rcp.Vdc", allow_none=True)
        fw_templates = FieldList('rcp.FirewallTemplate', allow_none=True)
        network = Field('rcp.Network')
        connected = Field(ConnectedObject, allow_none=True)
        vm = Field('rcp.Vm')
        router = Field('rcp.Router', allow_none=True)
