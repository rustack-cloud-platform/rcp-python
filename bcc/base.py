...
def resolve(cls):
    if isinstance(cls, str):
        ...
        cls2 = '.'.join(cls2)  # bcc.StorageProfile -> bcc.storage_profile
...
class BaseAPI:
    ...
    def __new__(cls, *args, token: str = None, endpoint_url: str = '',
        ...
        instance.token = token or os.environ.get('BCC_API_TOKEN', cls.token)
        instance.endpoint_url = endpoint_url or os.environ.get(
            'BCC_API_URL', cls.endpoint_url)
    ...
    def _call(self, http_method, resource, **kwargs):
        ...
        for task_id in resp.headers.get('X-Bcc-Tasks', '').split(','):
