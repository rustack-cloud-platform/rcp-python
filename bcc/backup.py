...
class RestorePoint(BaseAPI):
   ...
   Args:
       ...
       vm (object): Сервер для которого создана точка :class:`bcc.Vm`
   """
    class Meta:
        id = Field()
        backup_type = Field()
...
class Backup(BaseAPI):
   ...
   Args:
       ...
       vdc (object): Объект ВЦОДа :class:`bcc.Vdc`
   ...
   """
    ...
    def get_object(cls, id, token=None):
        ...
        Returns:
            object: Возвращает объект диска :class:`bcc.Disk`
        """
    ...
    def get_restore_points(self):
        ...
        Returns:
            list: Список объектов :class:`bcc.RestorePoint`
        """
        if self.id is None:
            raise ObjectHasNoId
    ...
    def restore(self, restore_point, power_on=True, quick_restore=False):
        ...
        Args:
            vm (object): :class:`bcc.Vm`, сервер, который необходимо
                восстановить
            restore_point (object): :class:`bcc.RestorePoint`, точка
