...
class Network(BaseAPI):
    ...
    Args:
        ...
        subnets (object): Список объектов класса :class:`bcc.Subnet`
        mtu (int): MTU сети
    ...
    def get_object(cls, id, token=None):
        ...
        Returns:
            object: Возвращает объект сети :class:`bcc.Network`
        """
    ...
    def add_subnet(self, subnet):
        ...
        Args:
            subnet (object): Объект подсети :class:`bcc.Subnet`
        """
        if subnet.id:
        ...
        subnet = self._call('POST', 'v1/network/{}/subnet'.format(self.id),
                            ...
                            dns_servers=[])
        self.subnets.append(resolve('bcc.Subnet')(token=self.token, **subnet))
